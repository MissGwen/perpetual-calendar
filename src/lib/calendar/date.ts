...
export function getCalendarDate(date: Date, currentMonthDate: Date): CalendarDate {
  ...
  // 十二建星 - 使用库的 getZhiXing()
  const dayValue = lunar.getZhiXing();

  // 黄道黑道值神 - 使用库的 getDayTianShen() 系列方法
  const dayGod = lunar.getDayTianShen();
  const dayTianShenType = lunar.getDayTianShenType(); // '黄道' or '黑道'
  const isYellowRoad = dayTianShenType === '黄道';
